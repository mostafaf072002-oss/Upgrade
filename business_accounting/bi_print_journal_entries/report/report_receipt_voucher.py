...
class ReportReceiptVoucherAccountView(models.AbstractModel):
    _name = "report.bi_print_journal_entries.report_receipt_voucher"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'unit': payment.new_unit_id.name,
            'currency_rate':format( payment.line_ids[0].debit,",") if  payment.line_ids[0].debit else format( payment.line_ids[0].credit,","),
            'currency':payment.line_ids[0].currency_id.name,
            'project' : payment.new_project_id.name,
            'label' : label,
            'cash_type':payment.cash_type,
