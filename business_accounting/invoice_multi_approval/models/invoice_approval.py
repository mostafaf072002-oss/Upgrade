...
class InvoiceApproval(models.Model):
    ...
    approve_journal_entry = fields.Boolean(string="Approval on Journal Entries",
                                           help='Enable this field for adding the approvals for the Journal Entries')
    journal_entry_approver_ids = fields.Many2many('res.users', 'journal_entry_id', string='Journal Entry Approver', domain=lambda self: [
        ('groups_id', 'in', self.env.ref('invoice_multi_approval.group_approver').id)],
                                         help='In this field you can add the approvers for the Journal Entries')
